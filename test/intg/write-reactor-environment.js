...
/*
This script writes `./test/intg/globalsForBrowser.js`, which is used by the
in-browser integration tests to get their local environment. That file must be
generated by this script before the `parcel build` of `allTestsForBrowser.js`.
...
$LOG_LEVEL _may_ be set, but is not required.
If set, it should have one of these values:
  `trace`
  `debug`
  `info`
  `warn`
  `error`
  `fatal`.
If not set, $LOG_LEVEL defaults to `error`.
*/
...
if (!process.env.ACCESS_TOKEN) {
  errors++;
  console.log('$ACCESS_TOKEN must be defined');
  console.log(
...
if (!process.env.COMPANY_ID) {
  errors++;
  console.log('$COMPANY_ID must be defined');
  console.log(
...
writeFile('./test/intg/globalsForBrowser.js', envJs);
